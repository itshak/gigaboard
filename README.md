...
[![npm version](https://img.shields.io/npm/v/gigaboard?color=%23cb3837&label=npm&logo=npm)](https://www.npmjs.com/package/gigaboard)
[![license](https://img.shields.io/npm/l/gigaboard)](./LICENSE)
...
- Server rendering is a real export. `gigaboard/server` emits static board markup with zero client JS and the same DOM shape the interactive board hydrates over.
- Budgets are explicit: **1 React commit per move**, **0 React work per drag frame**, **< 24 KB gzip** for the interactive surface, and CI gates for size, render count, and frame time.
...
```bash
bun  add gigaboard @gigaboard/themes @gigaboard/pieces
npm  install gigaboard @gigaboard/themes @gigaboard/pieces
pnpm add gigaboard @gigaboard/themes @gigaboard/pieces
yarn add gigaboard @gigaboard/themes @gigaboard/pieces
```

`@gigaboard/themes` and `@gigaboard/pieces` stay separate, tree-shakable packages; `gigaboard` uses the green theme and Neo pieces by default so a bare `<Chessboard/>` does not render as an unfinished placeholder. React 18.3+ or 19 is required. The engine adapter is powered by `gigachess`, a pure JS/TS high-performance chess engine requiring zero WASM compilation; SSR-only static boards ship zero client JS.
...
Short names: **`ours`** = `gigaboard`, **`rcb`** = [`react-chessboard`](https://github.com/Clariity/react-chessboard) 5.10, **`cg`** = [`chessground`](https://github.com/lichess-org/chessground) 9.2 (wrapped with `chess.js` for rules, as Lichess does).
...
| Metric                                          |          `gigaboard` (React) |  `react-chessboard` (React) |  `chessground` (vanilla TS) |
|-------------------------------------------------|-----------------------------:|----------------------------:|----------------------------:|
...
**Single-board interaction (INP + drag/move storm) — all three are inside the good band.** Gigaboard's INP distribution is tight: p50 13.0 ms, p95 16.6 ms. `rcb` has the best absolute worst case at 18.8 ms vs Gigaboard's 45.7 ms first-move cold-path outlier. All three libraries stay an order of magnitude below web.dev's 200 ms "poor" threshold; none of these gaps are perceptible on modern hardware. Drag storm wall-clock is a three-way tie at ~3.33 s.

**Where the architecture pays off:**

- **Memory stability under sustained play.** Over a 500-ply random walk, Gigaboard's heap grows **+1.18 MB** (best), tied with cg at +1.90 MB. `rcb` grows **+2.97 MB** this run — variable across runs (observed 2.97 → 13.79 MB across repeated runs; rcb's heap retention is volatile, Gigaboard's is flat). During the 40-ply drag storm, rcb's heap peaks at 20.4 MB against Gigaboard's 5.93 MB — 3.4× Gigaboard's working set. This is the metric that shows up as "the tab feels sluggish after 30 minutes of analysis."
- **Multi-board scaling.** At 100 boards Gigaboard uses **2.66× less JS heap** (45 MB vs 121 MB) and **3.32× fewer DOM nodes** (16 040 vs 53 332) than `rcb`. Paint time per extra board: 3.5 ms vs 9.0 ms. For puzzle walls, opening trees, analysis previews, game-list boards — the gap widens per board.
- **Render ownership.** Gigaboard holds at exactly **1.00 React commits per move** (byte-level `useSyncExternalStore` per square). `rcb` averages 2.83 because a position update fans through more React work. This is the contract the rest of the architecture is built around.

**Honest losses:**

- **Worst-case INP.** `rcb` has a tighter worst-case interaction (18.8 ms vs Gigaboard's 45.7 ms outlier). Gigaboard's outlier is a single first-move-commit cold path on the first interaction after mount — first React store-commit from a user event, first piece-slot mount/unmount, first animation-planner diff. Improving it requires pre-committing a synthetic move on mount; the engineering cost to eliminate a 30 ms single outlier that happens once per page load didn't clear our bar. Fixes we *did* ship — pre-materialising CSS `::before`/`::after` pseudo-elements on every square at mount — eliminated the first-SELECT outlier (was 52 ms, now 14 ms) and tightened p50-p95 across the board.
- **DOM mutations per move.** Gigaboard emits 9.2, `rcb` emits 2.6 — `rcb` reuses piece elements across squares via transform, Gigaboard mounts/unmounts per affected square as a natural consequence of its per-byte subscription model. The mutation count doesn't affect wall-clock on a single board but is a legitimate architectural difference. Guarded by a CI test with budget ≤10 (`apps/benchmarks/bench/playwright/mutation-audit.spec.ts`).
- **Cold mount.** Chessground is ~8 ms faster to LCP (no framework).
- **Grid scaling footprint vs `cg`.** `cg` wins raw resource footprint at 100 boards (5.7 MB vs Gigaboard's 45.3 MB). The cost `cg` pays is a GPL-3.0 license and bring-your-own-rules engine. Gigaboard is the React option for teams that want similar rendering discipline without leaving React.
...
> See [BENCH.md](./BENCH.md#react-layer--gigaboard-vs-react-chessboard-510) for the current numbers with the caveat intact. The wall-clock numbers that matter for shipped code are in the head-to-head table above, measured under a real `vite build`.
...
The React layer needs verbose moves to decorate legal-target squares; this is where the engine gives the UI headroom for analysis flows, puzzle batches, and opening trees. The engine is pure TypeScript/JavaScript powered by `gigachess`, with zero WASM compile latency and instantaneous synchronous execution.
...
|---------------------------------------|----------------------|-----------:|----------------------------------------------------------------------|
| `gigaboard`                           | yes                  |  **20.35 KB** | the interactive board — hooks, drag, animation, drawable arrows, premoves, haptics, viewOnly. |
| `gigaboard/server`                    | no (RSC)             |   **2.37 KB** | diagrams, PGN viewers, shareable position URLs; zero client JS.   |
| `@gigaboard/core`                    | no                   |   **3.74 KB** | framework-agnostic state + engine adapter; plug into RN, Jazz CRDT. |
| `@gigaboard/pieces/{alpha,cburnett,chesscom,merida,neo}` | yes | **~660 B** / set | one image-backed piece set; `neo` is the React default. |
| `@gigaboard/themes/{blue,brown,green,wood}` | yes       | **~205 B** / theme | one CSS-variable record; `green` is the React default.                       |
...
The static board produces the same DOM shape as the interactive one, so you can hydrate an interactive `<Chessboard/>` over it without a layout shift. Pair that with `fallbackFen` and the user never sees a flash of empty board.

### Warming the engine at app boot

`preloadEngine()` initializes engine state ahead of component mount so the board is immediately ready to interact:
...
It is idempotent: multiple calls resolve the same Promise. Because `gigachess` is pure TypeScript/JavaScript, engine startup is fast, synchronous, and requires zero external binary fetches or WASM compilation.

---

## API

Signatures are TypeScript. Everything below is exported from `gigaboard`.
...
| `<StaticChessboard fen …/>`         | Server-only; `gigaboard/server`. No client JS, hydration-parity DOM.                |

### Engine lifecycle

| Signature                                                            | Notes                                           |
|----------------------------------------------------------------------|-------------------------------------------------|
| `preloadEngine(): Promise<void>`                                     | Start engine init ahead of component mount. Call once at app boot; idempotent. |

### Hooks

| Signature                                                            | Notes                                           |
|----------------------------------------------------------------------|-------------------------------------------------|
| `useChessGame(opts?): BoardModel \| null`                            | Owns a `BoardModel` + `gigachess` adapter for the component lifetime. |
...
### Core (framework-agnostic, `@gigaboard/core`)

The React layer delegates all state to `@gigaboard/core`. Import it directly when you're building a non-React renderer, React Native, or plugging a collaborative backend (Jazz CRDT, Yjs, Liveblocks).
...
- `createGigachessAdapter(fen?)` — default `EngineAdapter` wrapping `gigachess`. Swap for `chess.js` or a variant engine without touching the React layer.
...
Engine-level errors come from the adapter and propagate unchanged: `IllegalMoveError`, `InvalidFenError`, `DisposedError`.
...
**Legal-move cache, hash-keyed.** Caching `from → legal-targets` by `(hash, from)` costs nothing to key and pays off immediately — ≥ 95 % hit rate in typical play. Invalidation is implicit: a new position has a new hash. Wrong hash = cache miss = engine call = correct answer.
...
- **Differential fuzz.** Core state is verified byte-for-byte against `gigachess` at every ply across 1,000+ random legal games. Any disagreement (FEN, legal-move set, check / mate / stale / draw) is a P0.
...
| `@gigaboard/pieces/*` per set     |   2 KB |  **~660 B**    |
| `@gigaboard/themes/*` per theme   |   1 KB |  **~205 B**    |

A board-shipping app therefore costs roughly **`core` + `react` + the default Neo pieces + the default green theme ≈ 22.3 KB gzipped**. Comparables: `react-chessboard` ~40 KB gzip + `chess.js` ~12.9 KB gzip; `chessground` ~12.3 KB JS + ~4.3 KB CSS + `chess.js`.
...
- [`apps/benchmarks`](./apps/benchmarks) — Node microbenchmarks, React Profiler harness, and the 3-way Playwright browser bench (Gigaboard / rcb / cg).
...
- **Full-canvas renderer is not shipped.** Canvas piece layer is a scaffolding placeholder; DOM is the only live renderer in this release.
...
**From `react-chessboard`.** Drop-in concept, not drop-in API. Key differences: `position` is replaced by `game` (a `BoardModel` from `useChessGame`); the board owns its engine via `gigachess` instead of `chess.js`; moves arrive as `PackedMove` (a branded `u16`) instead of a verbose object, decode with `decodePackedMove(m)` when you need `from`/`to`/SAN; drag-and-drop is built in and doesn't depend on `@dnd-kit`.

**From `chessground`.** Gigaboard is a React library — use `<Chessboard/>` like any other component. `chessground`'s `set({ fen, turnColor, ... })` imperative API becomes declarative props. Arrows, promotion overlay, sound, and premoves are built in rather than user-supplied. The `gigachess` engine handles rules; you no longer need to wire `chess.js` alongside.

Both integrations produce identical DOM shape for the board itself — a direct swap produces no layout shift.
...
bun -F @gigaboard/docs dev            # docs site → http://localhost:3000
```
...
- **[`ultrachess-react`](https://github.com/yahorbarkouski/ultrachess-react)** by Yahor Barkouski (MIT) — upstream library from which Gigaboard was forked.
- **[`gigachess`](https://github.com/itshak/gigachess)** — high-performance JS/TS chess engine providing O(1) hashing, magic bitboards, and fast move generation.
- **[`chess.js`](https://github.com/jhlywa/chess.js)** (Jeff Hlywa) — the JavaScript lingua franca of chess on the web, our baseline across every benchmark.
