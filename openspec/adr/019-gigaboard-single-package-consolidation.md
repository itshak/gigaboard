...
- Release automation simplified to single package: `release-please-config.json` tracks only `packages/react` (`gigaboard`, component `gigaboard`); `.release-please-manifest.json` is `{"packages/react": "0.0.1"}`; `release.yml` publishes one provenance-enabled artifact.
